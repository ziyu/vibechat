...
- `pnpm dev` starts backend (8002), web (8001), site (8003), admin (8005), and Space Runtime (8007) together; `pnpm dev:web` runs just backend + web. Docs is separate: `pnpm dev:docs`. Entry points: web/Spaces `http://localhost:8001/spaces`, site `http://localhost:8003/`, admin `http://localhost:8005/admin`. The web/site/admin apps proxy `/api` and `/v1` to the backend on `BACKEND_ORIGIN` (8002), so the backend must be up for authed screens to work.

### Chat backend (Matrix/Synapse) is optional and fails closed

- Without Synapse configured, `/spaces` intentionally shows the unavailable Space service state instead of loading fixtures — this is expected, not a bug. Auth, onboarding, product profile, account/billing, admin, and site all work without Synapse.
...
This project is indexed by GitNexus as **vibechat** (9204 symbols, 19649 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
| Resource | Use for |
|----------|---------|
| `gitnexus://repo/vibechat/context` | Codebase overview, check index freshness |
| `gitnexus://repo/vibechat/clusters` | All functional areas |
| `gitnexus://repo/vibechat/processes` | All execution flows |
| `gitnexus://repo/vibechat/process/{name}` | Step-by-step execution trace |
