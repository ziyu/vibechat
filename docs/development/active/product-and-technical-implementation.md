...
- 本地 #40 lifecycle/security 已在真实 Synapse、双 Chromium、SQLite、Web/Backend/Runtime 和独立 external Engine 上闭环：Candidate→ready 双端收敛、fixed Revision publish、后续 Revision 不改写 Release、固定旧版本与长生命周期 Live Release 复读、Runtime 非 2xx 透传、Kernel 显式恢复、opaque sandbox/CSP、bridge nonce/sequence/rate/payload 与 App proxy credential/header 隔离均通过。完整 `chat-matrix-room.spec.ts` 4/4、产品状态 9/9、Fake failure 1/1、Default/Campfire Chat Core 2/2、member-history 1/1；AgentOS Apps/Core `streamStdin` 安装产物回归与 A3/A4 repository suite 7/7 通过。
...
- [x] S5 仓库实现完成：生产 control 只预检区域级 external Engine；Agent/build/serving 是三个独立 OS worker/Envoy pool，接入各自 credential scope、egress、quota 和 metrics。control 的 session 请求不携带 provider secret，只有 Agent worker resolver 注入 credential；production control 带 key、Agent worker 缺 key 或 build/serving 带 key 均失败关闭。双 Node replica harness 已覆盖 lease/fencing/session/Release/Outbox，真实 disposable Engine 上三类 pool 各两个 replica 同时 active，停止一个 build worker 后保持 `2/1/2`。AgentOS Apps/Core `0.2.15` 补丁已保持 Release guest stdin/RivetKit callback，repository suite 7/7 和本地完整 lifecycle 4/4 通过。新增的只读 recovery manifest 和 `test:a3-a4:target` 会交叉验证 D1/R2/Synapse、两个 Runtime、fencing、固定 Revision/Release 与恢复清单；真实跨宿主与 Engine 备份恢复仍需目标环境执行，不能由本地 filesystem Engine 代替。
- [x] S6 仓库实现完成：Claude Code ACP `0.2.7` 已作为第二真实 Adapter 通过与 Pi/fake 相同的 lifecycle contract，并进入 Runtime Registry；Definition 增加不可变区域共享/专属 execution pool policy。Agent governance service、Admin API 与 `/admin/agents` 已覆盖严格 SemVer 版本、freeze/unfreeze、Space binding/default switch、budget/tool/region/pool policy 和 bounded audit，且不接收或返回 credential/Project source。SQLite/D1 `0015/0016` 已实际应用，Admin Chromium E2E 6/6、普通用户权限 API 12/12、S6 定向 unit 73/73 和 Workers/D1 health 200 通过；目标 suite 已实现真实 Claude Conversation/Revision、专属 pool 缺失/恢复和双 Chromium 验收，但实际 Anthropic/external Engine 运行仍待凭据注入。
...
| A3 | Space Kernel、Project 与 Space SDK | §5–§9、§14 阶段 1–2 | Active | contracts/SDK、nullable `room_index` migration、空白/模板创建、Kernel 模板应用/历史 rollback、Revision `0018`、Default/Custom Chat Core contract、member Mention/历史分页、Runtime、Backend gateway、真实 blank/apply/rollback、Chat/Mention/pagination、kick/leave，以及 #40 Candidate/publish/App proxy/bridge trust boundary E2E | 生产 R2/Object Store、external Engine 跨宿主、双 Runtime 接管与备份恢复 |
| A4 | Agent Adapter、Space Dev 与发布 | MVP §6、§7、§10、§14；[Agent/AgentOS 设计](../../stable/designs/agent-architecture-and-agentos-deployment.md) | Active | Product DB Definition/Binding/session/audit、完整 Pi/fake/Claude Code lifecycle、Admin 治理、区域/专属 pool policy、结构化 Matrix Mention、virtual-user Matrix 回写、queue、cancel、credits settlement/refund、Candidate 隔离、Revision rollback、fixed Revision publish 与本地完整 lifecycle E2E | 目标区域 external Engine 的 Claude/专属 pool、真实跨宿主恢复与备份恢复 |
...
| 模板应用策略 | 空白 Space 与已有定制 Project 已验证保持 Chat、App State 与 Release；继续守住 production Object Store/跨宿主恢复回归 | A3 生产验收前 |
