...
- Agent 架构 S1–S5 仓库实现已完成：结构边界、provider-neutral contracts、Agent 领域权威、完整 lifecycle/cancel/restore、external Engine topology、三类 pool 和双进程接管 harness 已落地。默认仍只开放 Pi、没有第二条 queue、Runtime 不直连 Product DB；生产目标环境的真实 Synapse/D1/R2/external Engine 跨宿主和备份恢复是剩余验收条件，不把本地 filesystem Engine 写成生产证据。
