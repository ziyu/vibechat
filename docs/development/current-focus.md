...
8. 按[Space App 基础组件库设计](./space-app-component-library-design.md)建立 SDK 之上的 User/Chat/Agent 组件层；五个官方 Template 已使用普通 package import、精确版本/integrity、受管 Registry 和 Revision-local prepared artifact 收敛重复 Chat/UI 源码，Default/Focus 又分别以 full/dock 消费 Chat Recipe。组件库 `0.9.3` 已补齐 provider-neutral Agent activity view/controller、Queue/Activity element、只读 Panel Recipe 和旧 Chat build 节点兼容桥接；Panel Recipe 默认仅在真实 active/queued 状态挂载可见布局，idle 同时设置 `hidden`、`aria-hidden` 与 `display:none`，需要常驻状态的 Space 可显式 `showWhenIdle: true`。Default/Focus 相邻 `0.2.0` 已复用同一 context 显式消费并通过隔离 iframe 的 idle→working、390px、200% 字体、forced-colors/reduced-motion 2/2。真实 Runtime `turn_started/status/activity/queue_updated/completion` 也已归并到 injected SDK 的同一 `space.agent` snapshot，activity 原位更新、时间正序且最多保留最新四条，Node 24 Runtime/SDK/Agent/Host 定向 unit 4 files、20/20；经明确授权，Default `0.2.0` 的真实 Pi/DeepSeek 双浏览器 Revision E2E 1/1（25.5 秒）已通过，两个成员通过公共 Mention/Agent 组件看到同一 identity、stage、queue、tool activity、完成 idle 与 ready Revision，Release 保持不变且刷新后 Chat 正常。`0.9.3` 又已通过本地受管 Registry 正常鉴权端点发布，并在真实 Default Space 中确认 idle 不可见、结构化 Pi 对话成功及真实积分结算。真实部署 publish、不可变 Release、跨 Runtime 恢复、Focus 真实 Agent 回归和 screen reader 继续属于独立验证，不能以仓库路径、Host 固定 UI、单版本本地缓存、浮动运行时脚本、mock SDK 或仅 event-shape unit 替代。
