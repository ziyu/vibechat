...
- Agent 架构 S1、S2 已完成：Runtime 的 scheduler、processor、release、Adapter、composition 与 transport 责任边界已经拆定；provider-neutral Agent contracts、旧包兼容导出、`libs/space-agents` 领域服务、四张 PG/SQLite-D1 对称领域表、Pi seed/历史 binding 回填和现有 Turn nullable 固定字段已经落地。Wrangler 本地 D1 已从 `0000` 迁移到 `0014`，PostgreSQL 17 已验证 `0014` 及恢复后的完整 `0000 → 0014` journal，两个 dialect 的 repository contract 均通过。S3 仍为 Pending：invoke 尚未切到 binding 权威，enqueue 尚未固定 Definition/Adapter/session/policy/reservation snapshot，也未开放多 Agent 或完成生产共享 Engine。
