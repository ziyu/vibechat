...
- 官方目录现有 Default Chat 与四个差异化模板，共五个 `agentos-app-v1` Project；每个 Template 在仓库只维护一份普通的多文件 `app/` 工作项目和一个扁平 `releases.json`。Default、Campfire、Focus、Arcade、Postcard 当前 development 版本分别为 `0.1.7 / 0.1.5 / 0.1.7 / 0.1.3 / 0.1.3`；Default/Focus 固定 exact `@vibechat/space-app-components@0.8.1` 并通过语义化 `/recipes/inline` 消费 full/dock Recipe，另外三个 Template 继续固定 `0.7.4` 与 `/chat/inline`。五个 Template 只保留布局、主题、文案与场景逻辑，删除重复 message renderer、Composer 和 Chat 状态机；首版 Recipe 进一步收敛 controller snapshot、typed events、增量 render、unread/read receipt 与 lifecycle。compact More 在支持 Popover 的浏览器进入 top layer，原生 light-dismiss、移动 `::backdrop` 与下一帧焦点恢复不受 Template `transform/backdrop-filter/overflow` 影响；旧浏览器保留 fixed/backdrop fallback。Runtime 只在 prepared Candidate/Revision 中 materialize verified package，并用独立 artifact pointer 持久化；stored source 不改写。Git 不保存逐版本编译目录，本地使用 gitignored exact-version Registry cache 同时解析 `0.7.4`/`0.8.1`，并可从已验证 prepared Project 恢复历史 package；每个供生产 Space 使用的组件版本仍必须先发布到 managed Registry/Object Store，公共 npm 只是可选 mirror。
- Default Chat 的 package 迁移已进入真实本地 Runtime：较早的 `0.5.0` prepared artifact `sha256:2548105d1ea72db8…` 已完成完整开发栈冷启动恢复；当前 `0.8.1` Recipe 的 Default/Focus 与继续固定 `0.7.4` 的 Campfire/Arcade/Postcard 又在同一真实 Matrix + Rivet/AgentOS 链路完成最终回归。`chat-matrix-room.spec.ts` 当前整文件 6/6，通过创建、发送、回复、Reaction、刷新、恢复、抽屉 unread、Popover top layer、Escape/外部点击/焦点恢复、Arcade 徽章、Postcard 卡片状态持久化和 390px 全宽布局；Campfire、Arcade 与 Postcard 均保留 active transform 与 `blur(26px)`，未再触发 Chromium renderer crash。仍未生成不可变 Release；生产 managed publish/Object Store、跨 Runtime、双 Chromium 同房交互和完整 a11y 矩阵继续待验证。
