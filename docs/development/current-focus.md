...
- 官方目录现有 Default Chat 与四个差异化模板，共五个 `agentos-app-v1` Project；每个 Template 在仓库只维护一份普通的多文件 `app/` 工作项目和一个扁平 `releases.json`。四个差异化模板当前仍为 `0.1.0 → 0.1.1 → 0.1.2`；Default Chat 已相邻追加 development 版本 `0.1.5`，通过 exact `@vibechat/space-app-components@0.7.0`、`space-app-dependencies.json` 和语义化 `/chat/inline` package import 使用 Chat 模块，并删除相对 vendor module、重复 message renderer、Composer 与 Chat 状态机。Host 显式下发 Chat permissions；交互 Timeline 只保留 canonical Reaction 与 compact More，候选 Reaction/回复/编辑/删除/重试渐进披露，相邻同作者消息按五分钟分组，桌面浮层和移动 action sheet 由公共组件负责。Default 不访问组件 Shadow DOM，read receipt 在 Chat 可见时按最新消息去重同步。Runtime 只在 prepared Candidate/Revision 中 materialize verified package，并用独立 artifact pointer 持久化；stored source 不改写，旧无 managed dependency 的 Space 保持原 Revision ID 算法。Git 不保存逐版本编译目录，每个供生产 Space 使用的组件版本都必须先发布到 managed Registry/Object Store；`0.7.0` 当前只有仓库 release lock 和本地 package 证据，尚未完成生产 managed publish。公共 npm 只是可选 mirror。`src/index.ts` 仍只负责 Runtime/handler 装配；Artifact/Space Revision/Dev Preview/Agent 编辑均识别并校验完整项目树。Version 只引用按 hash 寻址的不可变 artifact，历史源码不按版本复制；独立发布/部署从统一 Registry/Object Store 取 artifact。共享协议与 codegen 强制首版、相邻 SemVer、最高 current、非空升版和最新源码 hash 一致；旧 `builtin` v1–v5 与误用的 `5.0.0` ID 仅作开发数据读取 alias。
- Default Chat 的 package 迁移已进入真实本地 Runtime：较早的 `0.5.0` prepared artifact `sha256:2548105d1ea72db8…` 已完成完整开发栈冷启动恢复；当前 `0.7.0` / `space-default@0.1.5` 又在同一真实 Runtime 生成 ready Revision `04b4b51`。`chat-matrix-room.spec.ts` 当前整文件 3/3，通过真实 Matrix 发送/回复/Reaction/刷新/恢复，并独立覆盖 compact More 跨周期刷新、Delete 确认/焦点和 390px action sheet。仍未生成不可变 Release；生产 Object Store、跨 Runtime、双 Chromium 同房交互和完整 a11y 矩阵继续待验证。
