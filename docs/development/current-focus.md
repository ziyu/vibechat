...
8. 按[Space App 基础组件库设计](./space-app-component-library-design.md)建立 SDK 之上的 User/Chat/Agent 组件层；Default Chat 已使用普通 package import、精确版本/integrity、受管 Registry 和 Revision-local prepared artifact 收敛重复 Chat/UI 源码，下一步用真实 Matrix/AgentOS Dev/Release/冷启动验证后迁移抽屉式 Template，不能以仓库路径、Host 固定 UI 或浮动运行时脚本替代。
...
- 官方目录现有 Default Chat 与四个差异化模板，共五个 `agentos-app-v1` Project；每个 Template 在仓库只维护一份普通的多文件 `app/` 工作项目和一个扁平 `releases.json`。Default、Campfire、Focus、Arcade 已分别相邻追加 development `0.1.6 / 0.1.5 / 0.1.6 / 0.1.3`，并固定 exact `@vibechat/space-app-components@0.7.4`、managed integrity 与语义化 `/chat/inline` import；仅 Postcard 仍停留在 `0.1.2` 的 Template 自有 Chat 实现。已迁移 Template 只保留布局、主题与场景逻辑，删除重复 message renderer、Composer 和 Chat 状态机。compact More 在支持 Popover 的浏览器进入 top layer，原生 light-dismiss、移动 `::backdrop` 与下一帧焦点恢复不受 Template `transform/backdrop-filter/overflow` 影响；旧浏览器保留 fixed/backdrop fallback。Runtime 只在 prepared Candidate/Revision 中 materialize verified package，并用独立 artifact pointer 持久化；stored source 不改写。Git 不保存逐版本编译目录，每个供生产 Space 使用的组件版本都必须先发布到 managed Registry/Object Store；`0.7.4` 当前只有仓库 release lock 和本地 package 证据，尚未完成生产 managed publish。公共 npm 只是可选 mirror。
- Default Chat 的 package 迁移已进入真实本地 Runtime：较早的 `0.5.0` prepared artifact `sha256:2548105d1ea72db8…` 已完成完整开发栈冷启动恢复；`0.7.4` 的 Default/Campfire/Focus/Arcade 又在同一真实 Matrix + Rivet/AgentOS 链路完成最终回归。`chat-matrix-room.spec.ts` 当前整文件 5/5，通过创建、发送、回复、Reaction、刷新、恢复、抽屉 unread、Popover top layer、Escape/外部点击/焦点恢复、Arcade 徽章状态持久化和 390px 全宽布局；Campfire 与 Arcade 均保留 active transform 与 `blur(26px)`，未再触发 Chromium renderer crash。仍未生成不可变 Release；生产 Object Store、跨 Runtime、双 Chromium 同房交互和完整 a11y 矩阵继续待验证。
