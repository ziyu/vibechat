...
7. Agent 架构 S4 已完成并进入 S5：先实现生产外部 Engine mode/endpoint、Runtime replica identity 和 Agent/build/dev/serving pool 的结构与失败关闭，再建立两个 Runtime replica 共用一个外部 Engine 的 integration harness。两个 `SpaceInstanceServer` 共用 SQLite control plane 的确定性故障注入已覆盖 M1→Publish→M2、active owner 超时接管、旧 owner fenced write 与 outbox ACK 丢失重放；只有再用独立 Runtime 进程和真实 Synapse/D1/R2/AgentOS 证明 Matrix reply、credits、Revision/Release 均无重复，才把生产接管标为完成。
...
- Agent 架构 S1–S4 已完成：Runtime 结构边界、provider-neutral contracts、Agent 领域表和 Definition/Binding 调用权威已落地，现有 Turn 固定 Definition/Adapter/session/policy/Project/reservation snapshot。生产 Registry/Turn processor 已切到 Pi/Fake 共用的完整 lifecycle 与 strict event stream；Product DB session summary/ref/hash、restore/rebuild、bounded audit 和 cancel control 都经过 Backend internal API，并由 active Turn、lease/fencing 拒绝旧 owner 写。成员取消、Adapter Abort/cancel、usage 缺失失败退款和 Candidate repair 失败保护已接入唯一收口；定向单测 133/133、真实 Synapse collaboration E2E 3/3 和 Workers/D1 health 200 通过。默认仍只开放 Pi且没有第二条 queue；S5 已开始外部 Engine/replica/pool 结构切片，但生产共享 Engine、物理 pool 隔离和双进程 D1/R2 接管仍未完成。
