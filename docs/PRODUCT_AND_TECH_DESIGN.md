...
V1 不开放用户评分和评论。排序由人工精选、使用量、崩溃率、加载性能和举报情况决定。
...
V1 不制作图形化 Studio，提供 CLI、SDK、文档站和本地模拟宿主。
...
- Better Auth 的认证账号关联数据；V1 不由产品代码直接读写。

`verification`

- Better Auth 核心 verification schema；V1 配置 secondary storage 后，Email OTP 的短期验证值存入 Redis，并在写入前通过 `storeOTP` 哈希。

V1 显式配置 `session.storeSessionInDatabase: true`，因此活动 session 的权威存储仍为 PostgreSQL；Redis 仅作为 Better Auth 短期 verification 与 rate-limit 存储。产品 migration 不复制 Better Auth 表，也不把 Redis 用于任何耐久产品资料。
...
- V1 默认限制：图片 20MB，普通文件 100MB；管理员可配置。

### 10.4 搜索

- V1 无 E2EE，使用 Matrix room search 作为消息搜索基础。
...
- Email OTP 由 Better Auth plugin 生成和校验，V1 配置为 6 位、5 分钟有效、最多 3 次尝试、重发时轮换，并使用 `storeOTP` 哈希存储。
...
| V1 无 E2EE                                          | 用户对隐私能力预期不一致               | 明确产品说明；数据最小化；将 E2EE 作为独立后续架构项目                            |
...
当前仓库为空，本文件作为第一份产品与技术基线。环境曾引用 `RTK.md`，但该文件当前不存在；如果后续补充，开始实施前必须重新检查其中约束并更新本文档。

当前工程启动决策：以前端骨架为先，产品 Web 应用位于 `apps/web-app`，采用 TinyShip 的 TanStack Start、TanStack Router 和 Vite；暂不实现或接入产品后端、数据库与认证，待骨架建立并验收后再单独评审后端技术栈。
