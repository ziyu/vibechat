...
> 状态：可交互前端基础切片  
> 范围：TanStack Start Web 应用  
> 不包含：Matrix、Better Auth、产品后端、iframe Runtime 与生产数据

## 目标

这轮实现把 `PRODUCT_AND_TECH_DESIGN.md` 中的产品信息架构变成可以直接运行和验收的前端宿主，同时保持后端选型暂缓这一既有决策。它用于验证响应式布局、页面关系、交互契约和未来服务接入点，不把浏览器 fixture 数据描述为正式消息服务。
...
验收场景记录在 `tests/e2e/TEST-CATALOG.md` 的“聊天宿主基础功能”，自动化实现位于 `tests/e2e/specs/chat-foundation.spec.ts`。
