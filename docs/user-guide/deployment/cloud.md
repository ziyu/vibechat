...
本指南介绍如何将 Vibe Chat 项目部署到各种云平台，以及通用的服务器配置。
...
```toml
[build]
  base = "apps/next-app"  # 或 "apps/nuxt-app" 或 "apps/web-app"
...
if ! curl -f -s $SERVICE_URL > /dev/null; then
    ...
    curl -X POST -H 'Content-type: application/json' \
        --data '{"text":"🚨 Vibe Chat service is down!"}' \
        $SLACK_WEBHOOK
...
DATABASE_URL=postgresql://user:password@staging-db:5432/vibechat_staging

# .env.production
NODE_ENV=production
APP_BASE_URL=https://yourdomain.com
DATABASE_URL=postgresql://user:password@prod-db:5432/vibechat_production
```
    ...
    steps:
      ...
      - name: Deploy to server
        run: |
          ...
          ssh user@server 'pm2 restart vibechat-next'
```
