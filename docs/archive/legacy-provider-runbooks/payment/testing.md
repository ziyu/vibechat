...
> 归档原因：活动 Backend 已移除 provider SDK、支付 Webhook 与用户购买 API；恢复任何 provider 前必须重新核验本文命令、密钥和回跳契约。替代入口为[归档说明](../README.md)和仍在使用的[Admin 动态定价 Runbook](../../../stable/runbooks/payment/dynamic-pricing.md)。

本文档介绍如何在本地开发环境中测试支付功能，包括各支付平台的测试模式配置和 Webhook 调试方法。

> **前置条件（历史）**：原支付配置位于同一归档目录的 `overview.md`。
...
📚 **相关文档**：
- [归档支付配置概览](./overview.md) - 历史支付方式和计划
- [归档 Dodo Payments 配置](./dodo.md) - 历史 Dodo Payments 配置
- [活动积分账本 Runbook](../../../stable/runbooks/credits.md) - 当前 Admin 账本查询范围
