...
- [x] 对同一 Matrix `event_id` 重试 Agent command 只接受一次；相邻 Agent 请求按接收顺序执行，publish 作为单独屏障。
- [x] Agent 修改 Project 后 Candidate 通过校验并实时切换为当前 ready Revision；另一位成员看到相同 App 与 App state。
- [x] Publish 将固定 ready Revision 固化为不可变 Release；后续实时更新不改写该 Release。
- [x] Agent/Candidate 构建失败时继续运行最后 ready Revision；Kernel Bar 可以恢复 Default Chat App，Chat Core 数据不受影响。
...
- [x] Candidate 成功后对同一 Space 的双浏览器实时切换 ready Revision；失败保留最后 ready App；Publish 只固化固定 Revision，不把 Space 从“试验”变成“可用”。
...
- [x] App 文档代理对 Runtime 非 2xx 保持原始失败状态，不在 Template Project 之外合成 Default Chat HTML；Candidate 使用隔离的版本实例构建，失败后原 ready Revision 在 iframe 重载与页面刷新时仍可按固定版本读取。恢复 Default Chat App 只能由 Kernel 的显式恢复操作创建新的受管 Revision。
- [x] App 无法覆盖/伪造 Kernel Bar，不能读取 Matrix token、伪造身份、改写 Mention 解析、绕过屏蔽/ACL/计费或建立第二条消息 timeline。

本轮 lifecycle/security 联合验收固定为一个真实 Synapse Space、两个独立 Chromium Context 和同一条 Product DB/Runtime 权威链路：先记录 ready Revision `R1` 与已有 Published Release `P0`，通过 Kernel Template Candidate 切换到 `R2`，显式发布时固定 `R2`，再切换到 `R3` 并断言 Published Release 仍指向 `R2`；旧 `R2` 必须继续能由固定 `version` URL 读取。对不存在但格式合法的 Revision 请求必须保留 Runtime 非 2xx、JSON 错误体和原状态，不注入 Space SDK 或 Default Chat HTML；两个浏览器的 iframe reload/页面刷新仍挂载 Project 当前 ready Revision。随后由 Kernel 显式恢复 Default Chat，产生新的 ready Revision `R4`，同时保持固定 Published Release、Matrix timeline、成员和 App State。

同一验收还必须验证 opaque sandbox 与版本化 bridge 的信任边界：Kernel Bar 始终位于 iframe 外；App 不能访问 parent DOM、Cookie、Matrix token、Runtime 内部 credential 或 provider credential；Host 只接受当前 iframe `contentWindow`、本次随机 nonce、严格单调 sequence、受限速率和合法 action/payload。重放、错 nonce、未知 action、`app.publish`、伪造 sender/member/agent/Space/Release 字段和跨 Room Mention 必须在进入高权限服务前拒绝；成员身份始终由 Host/Backend 覆盖，Chat 写入只进入当前 Matrix Room，Agent 调度仍只来自确认后的结构化 Matrix Mention。拒绝路径不得新增 Space Turn、Outbox、credits reservation/settlement/refund、Release 或第二条消息存储。

2026-08-28 lifecycle/security 完成证据：

- Node 24.19.0、本地 SQLite、真实 Synapse `8008`、Web `18101`、Backend `18102`、Runtime `18107`，以及使用独立临时数据库和 Rivet 默认 `default` region 的 external Engine `16520` 上，新增 lifecycle/security 双 Chromium 场景聚焦运行 1/1（3.1 分钟）、完整 `chat-matrix-room.spec.ts` 4/4（7.0 分钟）通过。
- 同一真实 Space 中，Campfire `R1` ready 后显式 publish 固定 `R1`；Focus Candidate 切到 `R2` 时双方实时收敛而 Release 仍为 `R1`，`R1/R2` 固定 version URL 均可读；合法但不存在的 Revision 保留 Runtime 503 JSON 且不注入 SDK/Default Chat；双页刷新继续固定 `R2`；Kernel 显式恢复 Default Chat 后产生新 ready Revision，同时 Release、Matrix Chat、成员和 App State 保持。
- Kernel 始终在 iframe 外；opaque sandbox/CSP 阻止 parent DOM、Cookie、localStorage 与任意网络读取。App proxy 不再转发 Backend→Runtime 的短期 `Authorization` credential，只允许公开内容协商/缓存/range 请求头。Runtime 内部 API 无 credential 返回 401，伪造 client/member/author/agent/Space 字段由 Host/Backend 身份覆盖。
- Bridge 每次 iframe revision/reload 使用随机 nonce；Host 只接受当前 `contentWindow`、严格递增 sequence、64 KiB payload、10 秒 120 条命令和已知 action。错 nonce、replay、rate burst、未知 action 与 `app.publish` 均在 Host 拒绝；拒绝前后 Turn、Outbox、credits 与 Release 数量不变。
- `chat-space-agent-collaboration.spec.ts` 在 Runtime 显式启用 Fake Adapter 时失败保护 1/1 通过（另两条真实 provider 条件用例按设计跳过）；`chat-matrix-operations.spec.ts` 在干净 Engine 上 Default/Campfire 2/2（44.3 秒）通过，`chat-member-history.spec.ts` 1/1（2.5 分钟）通过。连续复用已运行完整 lifecycle 的同一 Engine 会留下 guest actor 并触发 Runtime capacity/guest RPC timeout，因此两组 Chat 回归使用新的独立 Engine DB 重跑，未把容量失败改写为产品通过。
- 定向 unit 9 个文件、38/38 通过，覆盖 contracts、bridge guard、App proxy header allowlist、SDK history、Runtime credential、Dev Preview、Publish 与 Restore；`space-app-contracts`、`space-app-sdk`、Web、Space Runtime TypeScript 检查通过。本组不包含生产 R2/Object Store/external Engine 跨宿主验收，A3/A4 因该生产门槛继续保持 Active。
...
- 结合 `chat-member-history.spec.ts` 的 Default/Campfire member Mention、跨 initial-sync 分页、失败保留和刷新去重 1/1 证据，#40 的 Chat Core contract 复选项已满足。本条保留该切片当时 Candidate/publish、App 代理失败和信任边界尚未闭环的事实；后续完成证据见 40.2 的 2026-08-28 lifecycle/security 记录。
...
- `libs/database/seed.ts` 直接插入的普通用户和 Alice/Bob/Carol Chat 测试用户会复用正式、幂等的 `grantNewUserCredits()`，避免绕过 Better Auth `databaseHooks.user.create.after` 后余额为零；`blank@vibechat.test` 继续显式保留空白，供空状态验收。
...
- [x] Default `0.2.0` full 与 Focus `0.2.0` dock 的生成文档都从 `recipes/inline@0.9.3` 注册 Agent Activity；idle 默认退出布局与辅助技术树，真实 working stage/queue/activity 到达后自动显示。同一 mock SDK/context 在 390px、200% 字体、forced-colors/reduced-motion 下无横向溢出、无 console error，且源码不再包含 `vcc-build` 投影。
- [x] 真实 `/v1/space-app-sdk` bridge 把 Runtime 的 `turn_started`、`status`、`activity`、`queue_updated` 和完成事件归并到同一 `space.agent` snapshot：活动按真实 tool call 原位更新，以时间正序保留最新四条，实际 Agent identity/stage/queue 触发组件 controller，完成后清除 build；无 active Turn 的孤立 progress fail closed，不创建第二个 SDK 或演示状态。
- [x] 真实 Matrix `@agent` Revision Turn 中，两个成员的 Default `0.2.0` iframe 同时通过 `vc-space-agent-activity` 看到实际 Agent identity、active stage、queue 和至少一条工具 activity；ready Revision 切换后两端恢复 idle 且 Chat/Release 不回归。该场景必须使用真实 provider，mock SDK/fake Agent 不能完成此项。
...
2026-08-28 Agent activity Template 迁移证据：兼容 patch `@vibechat/space-app-components@0.9.1` 的 source/browser/integrity 为 `sha256:511ca97c185edf3d4e5c2cb595005d9ecf3bf920a7a8ec63a21f414993e34785` / `sha256:92259060a408554cd5d91c22e6135380074bc957e366a82d2e5b35e75b3a9899` / `sha256:bf9d6ee624ca368380df425e9d284c9345ef255ecc5ac59c9233f58575ee6b68`。Default/Focus 相邻 development `0.2.0` 固定该 exact version/integrity，复用同一 SDK/context 挂载 full/dock Chat Recipe 与 Agent Panel Recipe，删除手写 `vcc-build` DOM/CSS；完整旧 build DOM 仍兼容、全缺失使用 detached hidden placeholder、部分缺失 fail closed。`space-template-agent-activity.spec.ts` 隔离 Chromium 2/2，通过动态 long stage、1 active/2 pending queue、两条 activity、polite live region、390px、200% 字体、forced-colors/reduced-motion、无动画、无横向溢出、Composer 可见和无 console/page error。经用户明确授权，`chat-space-agent-collaboration.spec.ts` 的真实 Matrix/Pi/DeepSeek 双浏览器 Default ready Revision 1/1（25.5 秒）通过：公共 Mention 组件产生结构化 Pi target 和 `/turns` 202，两端同时看到真实 Agent identity、working stage、1 active queue 与 tool activity，完成后恢复 idle、加载同一 Revision marker、Release 不变，Chat 发送与刷新恢复正常。当前证据仍不包含 Focus 真实 Agent、screen reader、生产 publish 或不可变 Release。

2026-08-28 idle 可见性与 seed 欢迎积分修正证据：`@vibechat/space-app-components@0.9.3` 的 source/browser/integrity 为 `sha256:a2a3eb0f095ad16ab34d6ca6f7f5d9552d14579c618aa687625da566fd9f2fe2` / `sha256:d8560e50a31d44f338efb4113a38fcf261587ffc2fa6bfba2401fe363f01ed81` / `sha256:9ad13ee6adee669d5a119668d34a67a8d1815451c5bc6999a8b80bf4edbb5081`；Default/Focus `0.2.0` 已重签到该 exact version/integrity，source/manifest 分别为 `sha256:b58d32037d74a0c548ce19a73fda90c5503bfd9184c934e40df85c079313da50` / `sha256:298878233ce79fcefbecc39aff272a41ea991d882b373e2c527edfd202318117` 与 `sha256:fadc4694a2a186a90a53116b199b39de9bde5200e47cf5deca4647a9cf3aba0d` / `sha256:0e85b4d7d9a97228a64b4370f738603170003d1abd0d01f2141d9afa19081ea4`。新建真实 Default Space 检查到 component `0.9.3`、idle `hidden`/`aria-hidden=true`/`display:none`，随后通过公共 Mention 发送 `@pi` 并收到“组件正在正常工作。”；当前 seeded 普通账号使用正式交易 `signup:welcome:user_8s4mjlfyk0d` 获得 1000 credits，本次真实调用产生 -4 reserve 与 -1 settlement，余额为 995。`0.9.3` 已经本地 managed Registry 正常鉴权 publish；生产部署 publish、不可变 Release、Focus 真实 Agent 和 screen reader 仍未覆盖。

User/Member 公共层 P0 DOM/API 验收场景（`0.10.0`，先写 Spec，再迁移 Template）：

- [x] `createSpaceUserDirectoryController(context)` 只复用注入的 `SpaceAppClient`，以 SDK 既有顺序投影 `self + members` 并订阅 `members`；等价 identity/presence 更新不重复通知，`dispose()` 幂等释放 listener，不创建第二个 SDK、Matrix client、timer 或 observer。
- [x] `vc-space-user-presence` 复用公共 StatusDot，同时提供在线/暂离/离线文字；`vc-space-member-list-item` 组合 Avatar、Name、Presence，长 display name/handle 在 390px 和 200% 字体下安全换行，selected/disabled/density 均有文字或原生状态语义。
- [x] `vc-space-member-list` 使用原生 listbox/option 或等价 button/list 语义，空目录有稳定文字状态；ArrowUp/ArrowDown/Home/End 移动 roving focus，Enter/Space 发送 bubbling/composed `vc-space-member-select` typed event，disabled item 不可选择，交互目标不小于 44px。
- [x] `vc-space-mention-target-item` 只呈现结构化 `SpaceMentionTarget` identity，明确 member/Agent 类型和 unavailable 文字状态，不扫描 handle、不触发 Agent。现有 `vc-space-mention-menu` 内部复用该元素，同时保持 `vc-space-mention-select`、ArrowUp/ArrowDown/Enter/Escape、结构化 `mentionIds` 与现有消费方 API 不变。
- [x] `/user` 保持 SSR-safe、side-effect-free 和领域 tree-shaking；`/register/user` 与聚合 registrar 可重复注册全部新元素。离线 catalog 使用同一 Member/Mention DOM 覆盖两套 token 主题、空状态、长文、390px、200% 字体、forced colors、reduced motion 和无 console/page error。
- [x] 本切片只建立组件库/API/catalog/unit/browser 基线，不自动升级官方 Template、既有 Project/Revision/Release；首个真实 Template 消费留给相邻 patch 显式固定 exact version/integrity，并单独验证成员目录与 Chat 行为不回归。

2026-08-28 User/Member `0.10.0` 组件库证据：source/browser/integrity 分别为 `sha256:21e5a30003d9e1ce5761381fd6cda151f39246388ead5c46b65f811612beddfe` / `sha256:c7d5199ecc59e1072cfacc792068bbd223483b9112c877ee5b001c107c94037f` / `sha256:61920297d2b7ef56c9c8b5a0a6571b8480b405e8f5dd2bd60cd97ea4ea82ad03`。公开实现按 identity/directory/mention/register 四层拆分；package unit 6 files、39/39，typecheck、managed build、semantic export/integrity 与 bundle gate 通过，`user.js` gzip 7,870 bytes。离线 catalog Chromium 1/1 覆盖两套主题、listbox/option、ArrowUp/ArrowDown/Home/End、Enter/Space typed select、disabled skip、空状态、member/Agent/unavailable、390px + 200% 字体、container-query 重排、forced colors、reduced motion、44px 最小目标、无横向溢出和无 console/page error；Impeccable detector 返回空结果。本轮没有修改任何 Template exact dependency、Project、Revision 或 Release，首个真实 Template 消费仍属于下一切片。

Campfire User Directory 首个 Template 消费验收场景（`space-campfire@0.1.6` / components `0.10.2`，先写 Spec，再实现）：

- [x] 新增语义化 `@vibechat/space-app-components/user/inline`，其 metadata 与独立 User inline bundle、package version、source hash 和 artifact hash 一致；普通 bundler 继续使用 side-effect-free `/user`，Registry object key、版本目录、仓库相对路径和 CDN 不进入 Project 源码。
- [x] `space-campfire@0.1.6` 只升级 exact component dependency/integrity 并在 App client 注入 User inline source；同一个 `/v1/space-app-sdk` 导出被传给 `createSpaceComponentContext` 和 `createSpaceUserDirectoryController`，组件不创建第二个 SDK、Matrix client 或成员状态源。
- [x] 夜航电台用 `vc-space-member-list` 投影真实 SDK `snapshot.members`，保留 SDK 顺序、头像/姓名/handle/presence、空状态与 `radio/listening` presence 更新；删除 Template 手写 `innerHTML + escapeHtml` 成员 pill renderer，Chat 抽屉与场景主题不重构。
- [x] Template unit 固定 `0.1.6`、exact `0.10.2`/integrity、`/user/inline` import 和新的 immutable source/manifest lock；既有 `0.1.5` Version、Project、Revision 与 Release 不被改写。
- [x] 隔离 Chromium 走查验证 SDK 成员 snapshot/update 通过公共 controller/MemberList 渲染，且无 console/page error；组件 catalog 的既有 Chromium 同时覆盖长名字、空目录、390px、200% 字体、键盘 focus、forced colors、reduced motion和无横向溢出。
- [x] 在本地 Synapse 和完整开发栈可用时，运行 `chat-matrix-room.spec.ts`，确认 Campfire Chat 发送、回复、Reaction 与刷新恢复不退化。
...
- 本证据只覆盖 `0018` 基线后的历史；迁移前已失去 pointer 的对象不推断、不恢复。真实 R2/external Engine 跨宿主继续保持未完成；本地 #40 lifecycle/security 已由 40.2 的后续记录闭环。
...
| 日期 | 应用 | 通过 | 失败 | 跳过 | 备注 |
|------|------|------|------|------|------|
| 2026-08-28 | Space App User Directory + managed Registry + five official Templates + Matrix | 6 | 0 | 0 | 隔离 SQLite、真实 Synapse 与全新 Rivet Engine；`0.10.2` Registry Published + Verified、90 文件 exact resolve；Campfire `0.1.6` 的真实成员目录、发送、回复、Reaction、刷新与恢复通过，Default/Focus `0.9.3` idle hidden 以及 Arcade/Postcard 回归通过 |
