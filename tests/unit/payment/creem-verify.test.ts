...
describe('Creem return URL verification', () => {
  ...
  beforeAll(async () => { ({ createPaymentProvider } = await import('@libs/payment')) })

  test('rejects an invalid return signature', async () => {
    const provider = await createPaymentProvider('creem')
