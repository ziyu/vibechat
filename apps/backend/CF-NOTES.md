...
Space Runtime always uses the existing `R2_BUCKET` binding for private, content-addressed `space-runtime/objects/<sha256>` source and managed package objects. Product DB stores Project/Revision/Release pointers, Instance snapshots, Turn/Lease/Fencing, Outbox rows and immutable managed package release records; deploy migrations through `libs/database/drizzle-sqlite/0014_purple_wasp.sql` for D1 before starting Space Runtime or publishing packages. The local Node Backend uses ignored `.data/space-runtime-objects` instead of R2 while preserving the same HTTP/Object Store contract.

Managed package publishing uses `PUT /v1/internal/space-app-managed-packages` with a dedicated `SPACE_APP_PACKAGE_PUBLISHING_TOKEN` and `space-app-package-registry` audience. Runtime resolution uses `POST` on the same path with the existing short-lived Backend callback credential; Runtime credentials cannot publish. Release jobs also set `SPACE_APP_PACKAGE_REGISTRY_ORIGIN` to the Backend origin. Production secrets belong in Wrangler secrets/CI and must not be committed. The stored object is the canonical JSON managed package envelope; npm tarballs are optional mirrors only.

For an isolated local D1/R2 verification, apply all D1 migrations and start Wrangler with the same persistence directory, then publish an exact version using the component package script. Verify that a duplicate returns `Verified`, content drift returns HTTP 409, and Runtime can resolve every locked historical version without a local package cache.
