...
Space Runtime always uses the existing `R2_BUCKET` binding for private, content-addressed `space-runtime/objects/<sha256>` source objects. Product DB stores Project/Revision/Release pointers, Instance snapshots, Turn/Lease/Fencing and Outbox rows, plus Agent Definition/Binding/Session/Audit metadata; migrate D1 through the latest journal entry, currently `libs/database/drizzle-sqlite/0018_fair_white_queen.sql`, before starting Space Runtime or enabling Agent domain reads. Migration `0017` makes Template lineage nullable for blank Spaces; Default Chat bootstrap must not write a false market lineage. Migration `0018` creates immutable per-Space Project Revision history and backfills only the current ready pointer as the history baseline. The signed internal `capture_recovery_manifest` action provides a bounded, content-free checkpoint for D1/R2/Engine recovery verification; it is not a backup endpoint and must remain behind `SPACE_RUNTIME_INTERNAL_TOKEN`. The local Node Backend uses ignored `.data/space-runtime-objects` instead of R2 while preserving the same HTTP/Object Store contract.
