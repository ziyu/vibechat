      ...
      POST: withCfDb(async ({ request }) => {
        ...
        try {
          ...
          const userOrder = await db
            .select({ id: order.id, userId: order.userId, provider: order.provider })
            .from(order)
          ...
          if (provider === 'wechat') {
            const wechatProvider = await createPaymentProvider('wechat')
          ...
          console.error('Error canceling order:', summarizePaymentError(error))
