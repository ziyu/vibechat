...
export function collaborationInstructions() {
  const componentRelease = getCurrentSpaceAppComponentManagedRelease();
  return [
    ...
    `The platform-managed UI dependency available to every Space is ${componentRelease.name}@${componentRelease.version}, pinned with integrity ${componentRelease.integrity}. Import semantic, tree-shakeable package subpaths such as '${componentRelease.name}/chat'; self-contained agentos-app-v1 HTML documents may use '${componentRelease.name}/chat/inline' for the prebundled browser module. Never import a Registry artifact path.`,
    `Before implementing common User, member directory, Chat, Composer, Mention, message action, Agent identity, queue, or activity UI, use the managed component catalog first: '${componentRelease.name}/core' provides the injected SDK context; '${componentRelease.name}/foundation' provides Avatar, IconButton, StatusDot and tokens; '${componentRelease.name}/user' provides User identity, presence, MemberList and structured mention targets; '${componentRelease.name}/chat' provides the Chat controller, Timeline, Composer, Mention, attachments, reactions and message actions; '${componentRelease.name}/agent' provides provider-neutral identity, queue and activity UI; '${componentRelease.name}/recipes' provides full Chat, drawer Chat and Agent Activity Panel composition.`,
    "Prefer those public controllers, elements and recipes over Template-local copies. Keep Template code focused on layout, theme, copy, scene state and Space-specific behavior. Customize through typed properties/events, slots, documented ::part surfaces and --vc-space-* tokens; never copy component source, query its Shadow DOM, or depend on undocumented nodes.",
    `When adding or retaining ${componentRelease.name}, package.json dependencies must use the exact version '${componentRelease.version}' and space-app-dependencies.json must use schemaVersion 'vibechat.space-app-dependencies/v1' with packages['${componentRelease.name}'] set to exactly { "version": "${componentRelease.version}", "integrity": "${componentRelease.integrity}" }. Keep all managed dependency versions exact, never use ranges or tags.`,
    "Never create vendor/vibechat-packages or vibechat.resolved-dependencies.json yourself. Those are verified build outputs generated by the Runtime Registry and must not appear in editable Project source.",
