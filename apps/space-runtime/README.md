...
组件源码使用 `/foundation`、`/user`、`/agent`、`/chat` 等语义化 package subpath；当前返回自包含 HTML 的 `agentos-app-v1` 可使用明确的 `/chat/inline` delivery adapter。Registry 的 object key、`/artifacts/*` 和版本目录均不得进入 Project import。Git 不保存组件的逐版本编译目录；每个可供 Space 使用的版本必须先把规范化 managed package object 发布到 Registry/Object Store，并登记不可变的 `name + version + integrity + projectFormats + objectKey/objectHash`。公共 npm tarball 仅可作为同一 package 的可选 mirror，不是 Runtime 准备 Project 的线上依赖。
