...
export const Route = createFileRoute('/$lang/admin/subscriptions/')({
...
function SubscriptionsPage() {
  ...
  const [data, setData] = useState<SubscriptionsData>({ subscriptions: [], total: 0 })
  ...
  useEffect(() => {
    async function fetchSubscriptions() {
      try {
        ...
        setData({ subscriptions: result.subscriptions || [], total: result.total || 0 })
      } catch (err) {
        console.error('Failed to fetch subscriptions', err)
        setError(t.admin.subscriptions.messages.fetchError)
      } finally {
        setLoading(false)
      }
    }
