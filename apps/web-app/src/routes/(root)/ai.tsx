...
export const Route = createFileRoute('/(root)/ai')({
  head: ({ match }) => seoHead(match.context.locale, (t) => t.ai.metadata),
  component: ChatPage,
})
  ...
  const handlePromptSubmit = async (message: { text?: string; files?: any[] }) => {
    ...
    if (!hasAccess) {
      toast.error(t.ai.chat.errors.insufficientCredits || 'Insufficient Credits', {
        ...
        action: { label: t.dashboard.credits?.buyMore || t.common.viewPlans, onClick: () => { window.location.href = '/pricing' } },
      })
      return
    }
